...
| 4 | **Which fields are read matters — but no policy ranking is claimed** | label-based simulation suggests random > scan > adaptive (0.907 at k=8); with the model in the loop the ordering changes and **no paired difference survives a multiple-comparison correction** (13–15 chips). The tool uses spread sampling because reading the *first* k fields called a 100%-bad chip *pass* with 0.94 confidence | `audit/adaptive_sampling.py`, `audit/03c_policy_model_in_loop.py`, `audit/03d_policy_bootstrap.py` |
