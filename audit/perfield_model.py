...
def predict(model, loader, device, tta=False):
    """P(bad) per field; tta averages the identity, horizontal and vertical flips."""
    model.eval(); ps, ys = [], []
    for x, y in loader:
        x = x.to(device, non_blocking=True)
        p = torch.softmax(model(x), dim=1)[:, 0]
        if tta:
            p = p + torch.softmax(model(torch.flip(x, dims=[3])), dim=1)[:, 0]
            p = p + torch.softmax(model(torch.flip(x, dims=[2])), dim=1)[:, 0]
            p = p / 3.0
...
def main():
    ...
    ap.add_argument("--arch", default="small", choices=["small", "large"])
    ...
    for ep in range(args.epochs):
        ...
        if (ep + 1) % 5 == 0 or ep == args.epochs - 1:
            pv, yv = predict(model, loaders["val"], device, tta=args.tta)
            mv = metrics(pv, yv)
    ...
    pt, yt = predict(model, loaders["test"], device, tta=args.tta)
    mt = metrics(pt, yt)
    ...
    (OUT / f"perfield_metrics_{tag}.json").write_text(json.dumps(
